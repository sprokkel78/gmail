...
class BrowserWindow(Gtk.Application):
    def __init__(self):
        super().__init__(application_id="com.sprokkel78.gmail")
        GLib.set_application_name("gmail")

    def do_activate(self):
        global win
        win=Gtk.ApplicationWindow(application=self, title="gmail")
        win.set_default_size(800, 600)
        win.connect("key-press-event", Key_Event)

        # Pick a directory for cache
        cache_dir = os.path.expanduser("~/.gmail/cache")
        ...
        cookie_manager.set_persistent_storage(
            "/home/" + user + "/gmail-cookies.db", WebKit2.CookiePersistentStorage.SQLITE
        )

        # Create WebView using this context
        webview = WebKit2.WebView.new_with_context(context)
        webview.load_uri("https://mail.google.com/")
